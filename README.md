...
- Monthly 모달 기능 (날짜 클릭 시 상세 할일 관리)
- Monthly 날짜 칸 점 표시 (완료/미완료 색상 구분)
...
- localStorage 연동
- TypeScript 타입 정의
- 컴포넌트 분리 (TodoInput, TodoItem, TodoFilter, MonthlyModal)
- 상단 네비게이션
- Weekly 페이지 (이번 주 날짜별 할일 CRUD, localStorage 연동)
- Monthly 페이지 (달력 그리드, 날짜별 할일 CRUD, localStorage 연동)
- Monthly 모달 기능 (날짜 클릭 시 상세 할일 관리, 추가/삭제/완료 체크)
- Monthly 날짜 칸 점 표시 (완료/미완료 색상 구분)
...
- v0.8.0 | Monthly 모달 기능 + 날짜 칸 점 표시 (2026-06-04)